...
const ScanLauncher = () => {
  ...
  return (
    <FormContainer>
      ...
      <FormMessage margin="0 0 0.75rem 0">
        If you have a text file containing URLs you would like to scan, you can
        upload it here.
        <br />
        The format should be one url per line in the file.
      </FormMessage>
      <form className="flex flex-col" id="scan-form" onSubmit={handleSubmit}>
        <FormLabel htmlFor="results-file">
          Upload Your Url Results Text File
          <br />
          (Filename should contain the hostname. IE: www.yahoo.ca.txt)
          <FormFileInput
            ref={fileInputRef}
            name="resultsFile"
            id="resultsFile"
        ...
        <FormLabel htmlFor="filePrefix">
          File Prefix for Your Scan Files
          <FormTextInput
        ...
        <FormButton id="scan-launch-btn" type="submit">
          Launch Scan
        </FormButton>
      </form>
      {showProgress ? (
        <FormMessage margin="0.75rem 0 0 0" id="scan-progress-message">
          Your Scan will now begin. It may take up to 30 minutes. Come back and
