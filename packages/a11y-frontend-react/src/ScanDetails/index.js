...
const ScanDetails = () => {
  ...
  const fetchScanDetailsFromAPI = () => {
    fetch(`${SITE_RESULTS_API}${siteName}/${scanFolder}/${scanDetail}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <ResultContainer>
      ...
      {violationList.map((violation, index) => {
        return (
          <div key={index} className="mt-4 p-4 border border-gray-400 rounded">
            ...
            <h4 className="text-lg mb-3">
              ...
              <span
                style={{
                  color: violation.impact === 'minor' ? 'green' : 'red',
                }}
              >
                {violation.impact}
              </span>
            ...
            <ol
              type="a"
              className="ml-4"
              style={{ listStyle: 'lower-alpha !important' }}
            >
              {violation.nodes.map((node, idx) => {
                return (
                  <li key={idx} className="mb-3">
                    <p className="mb-3">{node.failureSummary}</p>
                    <button
                      className="mb-3 rounded p-2 border-2 border-yellow-500 hover:border-gray-500"
                      id={`btn-${index}-${idx}`}
                      type="button"
                      onClick={(e) => toggleCodeView(`code-${index}-${idx}`)}
                    >
                      Show Target Markup
                    </button>
                    <pre
                      className="bg-blue-100 border border-gray-400 rounded p-3"
                      id={`code-${index}-${idx}`}
                      style={{
                        display: 'none',
