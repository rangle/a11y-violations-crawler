...
const ScanListing = () => {
    ...
    const fetchScanResultsFromAPI = () => {
        fetch(`${SITE_RESULTS_API}${siteName}/${scanFolder}`).then(res => res.json()).then(data => {
    ...
    return (
        <ResultContainer>
            ...
            <H2>Total violations for {siteName}: {totalViolations}</H2>
            ...
            <ol className="mt-3 list-decimal list-inside">
                {urlList.map(function (url, index) {
                    return <li key={index} className="leading-6"><BlueLink to={`./${scanFolder}/${url.resultFile.replace('.json', '')}`}>{url.siteUrl}</BlueLink></li>
                })}
            </ol>
        </ResultContainer>
    );
